...
def get_input_from_batch(batch):
    ...
    batch_size, max_enc_len = (len(enc_batch), max([max(s) for s in enc_lens]))
...
def print_custum(emotion, dial, ref, hyp_b, hyp_g, pred_emotions, comet_res, pred_dia_emotions, dia_emotion):
    res = ""
    res += "Emotion: {}".format(emotion) + "\n"
    res += "DiaEmotions: {}".format(dia_emotion) + "\n"
    if pred_emotions:
        res += "Pred Emotions: {}".format(pred_emotions) + "\n"
    if pred_dia_emotions:
        res += "Pred DiaEmotions: {}".format(pred_dia_emotions) + "\n"
    if comet_res:
...
def evaluate(model, data, ty="valid", max_dec_step=30):
    ...
    dia_a = []
    trg_a = []
    dia_acc, utt_acc, trg_acc = 0.0, 0.0, 0.0
    pbar = tqdm(enumerate(data), total=len(data))
    for j, batch in pbar:
        loss, ppl, cls_loss, topic_loss, accs, top_preds, comet_res, dia_emo  = model.train_one_batch(batch, 0, train=False)
        (dia_acc, utt_acc, trg_acc) = accs

        l.append(loss)
        ppl_a.append(ppl)
        topic_a.append(topic_loss)
        dia_a.append(dia_acc)
        utt_a.append(utt_acc)
        trg_a.append(trg_acc)
        pbar.set_description(
            "loss:{:.4f} topic_loss:{:.4f} ppl:{:.1f}".format(np.mean(l), np.mean(topic_loss), math.exp(np.mean(ppl_a)))
        )
        ...
        if ty == "test":
            ...
            for i, greedy_sent in enumerate(sent_g):
                ...
                temp = print_custum(
                    ...
                    comet_res=comet_res,
                    pred_dia_emotions=dia_emo,
                    dia_emotion=batch["dia_emotion_txt"][i]
                )
                results.append(temp)
    if ty == "test":
        ma_dist1, ma_dist2, mi_dist1, mi_dist2, avg_len = get_dist(res)
        print("ma_dist1\tma_dist2\tmi_dist1\tmi_dist2\tavg_len\t")
        print(
            "{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.2f}\t".format(ma_dist1, ma_dist2, mi_dist1, mi_dist2, avg_len))
        return np.mean(l), np.mean(topic_loss), math.exp(np.mean(ppl_a)), np.mean(dia_a), np.mean(utt_a), np.mean(
            trg_a), results, mi_dist1, mi_dist2, avg_len
    return np.mean(l), np.mean(topic_loss), math.exp(np.mean(ppl_a)), np.mean(dia_a), np.mean(utt_a), np.mean(trg_a), results
