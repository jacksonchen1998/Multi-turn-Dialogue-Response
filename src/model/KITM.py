...
from torch_geometric.nn import GatedGraphConv, GraphConv, GCNConv, GATConv
import pickle
import random
from src.model.ConvNTM import ConvNTM
...
class Graph_Encoder(nn.Module):
    def __init__(self, hidden_dim, is_bi_rnn, net='gcn', bn_gnn=True, dropout=0.5):
        ...
        if net == 'gcn':
            self.gnn = GCNConv(hidden_size, hidden_size).to(config.device)
        if net == 'graphconv':
            self.gnn = GraphConv(hidden_size, hidden_size).to(config.device)
        if bn_gnn:
            self.bn = nn.BatchNorm1d(hidden_size).to(config.device)
        if net == 'gat':
            self.gnn = GATConv(hidden_size, hidden_size).to(config.device)
        if net == 'mult_gcn':
            self.layers = nn.ModuleList([GCNConv(hidden_size, hidden_size).to(config.device) for _ in range(5)])
        self.bn_gnn = bn_gnn
        self.input_dropout = nn.Dropout(dropout).to(config.device)
...
class KITM(nn.Module):
    def __init__(
        ...
        self.conv_graph_encoder = Graph_Encoder(config.hidden_dim, self.bi_gru, net='gcn', bn_gnn=False, dropout=0.5)
        ...
        self.ntm_model = ConvNTM(hidden_dim=config.hidden_dim).to(config.device)
        self.att_layer = Attention_Layer(config.hidden_dim, is_bi_rnn=self.bi_gru)
    ...
    def get_topic(self, bow_tensor, pkl_path, v, lens, topk=200):
        # Load the word mapping from the pickle file
        with open(pkl_path, 'rb') as file:
            word_map = pickle.load(file)
        
        # Ensure the bow_tensor is a torch tensor
        if not isinstance(bow_tensor, torch.Tensor):
            bow_tensor = torch.tensor(bow_tensor)
        
        # Create a mapping from words in pkl_data to indices 0 to len(pkl_data)-1
        word_to_id = {word: idx for idx, word in enumerate(word_map.keys())}
        id_to_word = {v: k for k, v in word_to_id.items()}
        
        # Initialize the list to hold the mapped topics
        mapped_topics = []
        unk_id = v.word2index.get('UNK', None)
        pad_id = v.word2index.get('PAD', None)

        max_len = max(lens)
        
        # Iterate through each row in the bow_tensor
        for row in bow_tensor:
            # Get the topk indices
            topk_indices = torch.topk(row, topk).indices.tolist()
            
            # Map the indices to words, using 'UNK' if the word ID is not found
            mapped_words = [v.word2index.get(id_to_word.get(idx, 'UNK'), unk_id) for idx in topk_indices]
            
            # Append the mapped words to the result list
            mapped_topics.append(mapped_words)

        batch_topic = []
        start_idx = 0
        for topic_len in lens:
            # Concatenate all topics in the current batch
            concatenated_topic = [word for sublist in mapped_topics[start_idx:start_idx + topic_len] for word in sublist]
            
            # Pad to ensure the same length as max_len * topk
            padded_topic = concatenated_topic + [pad_id] * (max_len * topk - len(concatenated_topic))
            
            batch_topic.append(padded_topic)
            start_idx += topic_len

        # for topic in batch_topic:
        #     # Map each index in the topic back to its word using v.index2word
        #     topic_words = [v.index2word.get(index, 'UNK') for index in topic]
    ...
    def construct_bow_vector(self, vocab, pkl_path):
        with open(pkl_path, 'rb') as file:
            pkl_data = pickle.load(file)
        bow_vector = np.zeros(len(pkl_data))

        # Create a mapping from words in pkl_data to indices 0 to len(pkl_data)-1
        word_to_id = {word: idx for idx, word in enumerate(pkl_data.keys())}

        unk_id = len(pkl_data)  # Use len(pkl_data) as the unk_id index
        bow_vector = np.append(bow_vector, 0)  # Append an extra slot for UNK

        # Update the bow_vector based on the pkl_data and new mapping
        for word, percentage in pkl_data.items():
            if word in vocab.word2index:
                word_id = word_to_id[word]
                bow_vector[word_id] = percentage
                #print(f"Word '{word}' found in vocab, assigned to new ID {word_id}")
            else:
                bow_vector[unk_id] = percentage
                #print(f"Word '{word}' not found in vocab, assigned to UNK ID {unk_id}")

        return torch.tensor(bow_vector, dtype=torch.float)
    ...
    def save_model(self, running_avg_ppl, iter):
        ...
        model_save_path = os.path.join(
            self.model_dir,
            "KITM_{}_{:.4f}".format(iter, running_avg_ppl),
        )
    ...
    def forward(self, batch):
        ...
        new
        '''
        uttr_nums = [len(uttr) for uttr in batch["input_batch"]]
        # print(config.topic_num)
        # print(max(uttr_nums))
        padding_size = config.topic_num * max(uttr_nums)
        padding_tensor = torch.full((know.size(0), padding_size), fill_value=config.PAD_idx, device=config.device)

        # Step 4: Embed the padding tensor
        padding_embs = self.embedding(padding_tensor).to(config.device)

        # Step 5: Concatenate `know_embs` with the embedded padding tensor along the second dimension
        know_embs_padded = torch.cat([know_embs, padding_embs], dim=1)

        lstm_output, attn_scores = self.att_layer(lstm_output, uttr_nums)

        old_lstm_output = lstm_output

        bs = len(old_lstm_output)
        uttr_last_num = [0] + torch.LongTensor(uttr_nums).cumsum(0).tolist()
        input_uttr_embeddings = old_lstm_output

        x, edge_index = [], []
        for b in range(bs):
            x.append(input_uttr_embeddings[b, :uttr_nums[b], :])
            edge_range = list(range(uttr_last_num[b], uttr_last_num[b+1]))
            speaker_idx = [list(range(uttr_last_num[b+1]+i, uttr_last_num[b+1], config.speaker_num)) for i in range(config.speaker_num)]
            if edge_range:
        ...
        if edge_index:
            ...
            input_uttr_embeddings = torch.zeros(bs, max(uttr_nums), x.shape[-1], requires_grad=True).to(config.device)
            for b in range(bs):
                input_uttr_embeddings[b, :uttr_nums[b], :] = x[uttr_last_num[b]:uttr_last_num[b+1], :]
            lstm_output = input_uttr_embeddings # learn the role of listener and speaker
        else:
            lstm_output = input_uttr_embeddings # learn the role of listener and speaker
        ...
        new_lstm_output = lstm_output

        lstm_output = config.ratio_for_multirole_enc * new_lstm_output + (1 - config.ratio_for_multirole_enc) * old_lstm_output

        batch_bow = self.construct_bow_vector(self.vocab, config.pkl_path)
        config.bow_vocab_size = len(batch_bow)
        # Reshape self_batch_bow to [1, 1, 7533]
        batch_bow = batch_bow.contiguous().view(1, 1, -1)

        # Expand self_batch_bow to [16, 3, 7533] to match the shape of old_lstm_output
        batch_bow = batch_bow.expand(old_lstm_output.size(0), old_lstm_output.size(1), -1)

        topic_loss, topic_res = self.ntm_train_one_batch(batch_input=lstm_output, batch_bow=batch_bow, lens=uttr_nums, ntm_model=self.ntm_model, optimizer=self.optimizer)
        topic_id = self.get_topic(topic_res, config.pkl_path, topk=config.topic_num, lens=uttr_nums, v=self.vocab)
        topic_id_tensor = torch.tensor(topic_id, dtype=torch.long).to(config.device)
        
        # Get the topic embeddings
        topic_embs = self.embedding(topic_id_tensor).to(config.device)
        topic_know_embs = torch.cat((know_embs, topic_embs), dim=1)
        know_embs = topic_know_embs
        ...
        return src_mask, enc_outputs, (context_emo_loss, utt_emo_loss, trg_loss, topic_loss), (
    ...
    def ntm_train_one_batch(self, batch_input, batch_bow, lens, ntm_model, optimizer):
        assert len(batch_input) == len(batch_bow) == len(lens)
        batch_bow = batch_bow.to(config.device)
        #print("batch_bow", batch_bow.shape)
        optimizer.zero_grad()
        batch, recon_batch, mu, logvar = ntm_model(batch_bow, lens, batch_input)
        ...
        rec_loss, kl = self.loss_function(recon_batch, batch, mu, logvar)

        loss = config.w_recbow * rec_loss + config.w_KL * kl
        # if config.add_co_matirx:
        #     beta = ntm_model.get_beta()
        #     M = torch.load("processed_data/{}/co_matrix.pt".format(config.data_tag)).to(beta.device)
        #     loss_rec_M = F.binary_cross_entropy(torch.matmul(beta.T,  beta), M, size_average=False)
        #     if loss_rec_M <= config.target_rec_M:
        #         w_rec_M = 0
        #         unfix_model(ntm_model)
        #     else:
        #         tmp = (loss_rec_M-config.target_rec_M)/0.05
        #         if tmp <= 1:
        #             unfix_model(ntm_model)
        ...
        if config.max_grad_norm > 0:
            grad_norm_before_clipping_ = nn.utils.clip_grad_norm_(ntm_model.parameters(), config.max_grad_norm)
        optimizer.step()
        
        return loss.div(len(batch_bow)), recon_batch
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        (context_emo_loss, cls_loss, trg_loss, topic_loss) = cls_loss
        ...
        if self.is_eval:
            dia_emo = context_emo_logits.detach().cpu().numpy().argsort()[0][-3:][::-1]
            top_preds = trg_emo_logits.detach().cpu().numpy().argsort()[0][-3:][::-1]
            dia_emo = f"{', '.join([MAP_DIA_EMO[pred.item()] for pred in dia_emo])}"
        ...
        if not (config.woDiv):
            ...
            loss = emo_loss + 1.5 * div_loss + gen_loss + topic_loss
            c_cls = 1.5 * div_loss + gen_loss
        else:
            loss = emo_loss + gen_loss + topic_loss


        if train:
            loss.backward()
            self.optimizer.step()
        return gen_loss.item(), gen_loss.item(), emo_loss.item(), topic_loss.item(), accs, top_preds, comet_res, dia_emo
